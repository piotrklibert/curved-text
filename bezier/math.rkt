...
(provide
 ;; Not contracted because the overhead when calling is too big.
 ;; TODO: rewrite in Typed Racket
...
(begin-encourage-inline
  ...
  (define (naive-deriv^3 t a b c d)
    (let ([t-minus-one (- t 1.0)])
      (* (- 3) (+ (* a (square t-minus-one))
                  (* b (+ (* (- 3.0) (square t)) (* 4.0 t) (- 1.0)))
                  (* t (- (* 3.0 c t) (* 2.0 c) (* d t)))))))


  ;; ===========================================================================

  ;; Optimised functions for bezier curve computations. These are actually used
  ;; and exported.
...
(module+ test
  ...
  (let-syntax ([repeat-num (λ (stx) #'10000000)])
    (displayln (format "Computing bezier^3 ~a times took:" repeat-num))
    (time (for ([_ (in-range repeat-num)])
            (bezier^3 0.2345 20.0 30.0 40.0 120.0)))

    (displayln (format "Computing naive-bezier^3 ~a times took:" repeat-num))
    (time (for ([_ (in-range repeat-num)])
            (naive-bezier^3 0.2345 20.0 30.0 40.0 120.0)))

    (displayln (format "Computing deriv^3 ~a times took:" repeat-num))
    (time (for ([_ (in-range repeat-num)])
            (deriv^3 0.2345 20.0 30.0 40.0 120.0)))

    (displayln (format "Computing naive-deriv^3 ~a times took:" repeat-num))
    (time (for ([_ (in-range repeat-num)])
